...
<c:if test="${responses ne null}">
    <c:choose>
        ...
        <c:otherwise>
                ...
                <table>
                    ...
                    <c:forEach var="response" items="${responses}">
                        ...
                        <tr>
                            ...
                            <td>
                                <button form="changeResponseForm" name="${Constants.PARAMETER_RESPONSE_TO_CHANGE}" value="${response.id}"><fmt:message key="button.change"/></button>
                            </td>
                            <td>
                                <button form="deleteResponseForm" name="${Constants.PARAMETER_RESPONSE_TO_DELETE}" value="${response.id}"><fmt:message key="button.delete"/></button>
                            </td>
                        </tr>
                    </c:forEach>
                </table>
            ...
            <form id="changeResponseForm" name="changeResponseForm" method="post" action="${Constants.FRONT_CONTROLLER_SERVLET_VALUE}">
                <input type="hidden" name="${Constants.PARAM_COMMAND}" value="changeResponse">
            </form>
            <form id="deleteResponseForm" name="deleteResponseForm" method="post" action="${Constants.FRONT_CONTROLLER_SERVLET_VALUE}">
                <input type="hidden" name="${Constants.PARAM_COMMAND}" value="deleteResponse">
            </form>
        </c:otherwise>
    </c:choose>
</c:if>
