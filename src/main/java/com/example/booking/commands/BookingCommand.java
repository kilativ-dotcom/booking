...
public class BookingCommand implements Command{
    ...
    public AbstractPage execute(HttpServletRequest request) {
        ...
        Optional<Request> optionalRequest = RequestsService.creteRequest(user, checkInDate, checkOutDate, guests, comment, new SuiteClass(suiteClassId, suiteClass));
        if (optionalRequest.isPresent()){
            request.setAttribute(Constants.ATTRIBUTE_CREATED_REQUEST, optionalRequest.get());
        }
        return new RedirectPage(request.getContextPath() + "/welcome");
    }
}
